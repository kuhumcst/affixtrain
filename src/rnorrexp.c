...
static double/*float*/ wn[128],fn[128], we[256],fe[256];

#define RNOR (hz=SHR3, iz=hz&127, (fabs((double)hz)<(double)kn[iz])? hz*wn[iz] : nfix())
#define REXP (jz=SHR3, iz=jz&255, (    jz <ke[iz])? jz*we[iz] : efix())

/* nfix() generates variates from the residue when rejection in RNOR occurs. */

double/*float*/ nfix(void)
{
const double/*float*/ r = 3.442620f;     /* The start of the right tail */
static double/*float*/ x, y;
 for(;;)
